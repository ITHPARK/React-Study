...
리액트에서 요소의 리스트를 나열할 때는 Key 값을 부여해줘야함

리액트는 가상 돔을 이용해서 이전 가상돔과 이전 가상 돔에서 변경된 부분만 비교해서 변경을 한기 때문에
리스트를 나열할 때 키값을 넣어주면 바뀐 부분을 인식할 수 있다.

키 값으로 인덱스는 비추천? : 리스트 요소들의 순서가 변경되면 리스트 값들의 인덱스가 달라지기 때문에 비추천
